...
export function getMissingCognitoOidcConfig(): string[] {
  ...
  if (!resolveIssuerUrl()) {
    const hasDirectIssuer = Boolean(
      process.env.COGNITO_ISSUER_URL ?? process.env.NEXT_PUBLIC_COGNITO_ISSUER_URL ?? null,
    )
    const hasUserPoolId = Boolean(process.env.COGNITO_USER_POOL_ID ?? null)
    const hasRegion = Boolean(resolveRegion())

    if (!hasDirectIssuer && !hasUserPoolId) {
      missing.push("COGNITO_ISSUER_URL または COGNITO_USER_POOL_ID")
    } else if (!hasDirectIssuer) {
      if (!hasRegion) {
        if (!missing.includes("COGNITO_REGION")) {
          missing.push("COGNITO_REGION")
        }
      } else {
        missing.push("COGNITO_ISSUER_URL")
      }
    } else if (!hasUserPoolId) {
